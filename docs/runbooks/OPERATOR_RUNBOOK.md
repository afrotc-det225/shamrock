...
- Single-cell and multi-row Decision edits are processed row by row. If commander selections are visible in Excusals Management but missing from the backend or Attendance, run `SHAMROCK → Excusals → Reconcile management decisions`; it applies only missing or mismatched decisions and does not resend historical decision emails.
