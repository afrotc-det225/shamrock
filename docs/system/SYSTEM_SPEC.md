...
- Frontend formatting must never delete Sheets Table objects as part of normal formatting. It should update existing table objects in place when possible, apply normal cell-level data validation, then make a best-effort table ensure/style pass and reapply validation as a safety pass.
