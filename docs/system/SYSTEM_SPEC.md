...
- Frontend formatting must reset Sheets Table column types before clearing or applying normal cell-level data validation, because typed table columns block validation writes. Validation must be the final table-related operation so later table updates do not strip validation from the table body.
