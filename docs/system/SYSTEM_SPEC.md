...
- Directory frontend sync writes ordinary columns separately from `Photo Link`, applies file chips through the Sheets API, and then leaves the table column type unset. Cell smart chips do not require a `FILES_CHIP` table column type.
