...
- Each live-progress window must maintain only one polling chain and at most one in-flight `getShamrockProgress` request. Action completion must not start another loop. Active work polls no faster than approximately every five seconds, waiting states slow further, and failures use bounded backoff. Close, unload, page-hide, and hidden-window paths must prevent stale callbacks from scheduling more requests.
